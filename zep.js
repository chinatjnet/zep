Zepto=(function(){
	...
	$.fun={
		...
		index: function(elm){
			return elm ? [].indexOf.call(this,elm[0]||elm):
			[].indexOf.call(this[0].parentNode.children,this[0]);
    	},
		css:function(name,value){
			$.fun.each.call(this,function(i,elm){
				if(value==''){
					var reg=new RegExp(name+"\s*:.*?(;|$)","ig"),
						style=(elm.getAttribute("style")||'').replace(reg, "");
        ...
        toggleClass:function(name){
        	$.fun.each.call(this,function(i,elm){
        		$.fun.hasClass.call([elm],name)?$.fun.removeClass.call([elm],name):
        		$.fun.addClass.call([elm],name);
        	})
        	return this;
        },
		...
		on:function(event,callback){
			$.fun.each.call(this,function(i,elm){
				event.split(/\s/).forEach(function(event){ elm.addEventListener(event, callback, false); });
			})
			return this;
		},
		...
		unbind:function(event){
			$.fun.each.call(this,function(i,elm){
				event.split(/\s/).forEach(function(event){ elm.removeEventListener(event,elm.event); });
			})
			return this;
		},
	    trigger: function(event){
			$.fun.each.call(this,function(i,elm){
				var e; elm.dispatchEvent(e = d.createEvent('Events'), e.initEvent(event, true, false));
			})
			return this;
	    },
	...
	$.extend = function(target, src){if(!target)return target;for(k in src) target[k] = src[k];return target;}
...
;(function($){
	...
	Touch=function(){
		...
		function checkEvent(){
			...
			var _p=tPoint,_end=_p.end,_st=_p.start,target=_st.target,
			inter=_end.timestamp-_st.timestamp,
			xL=_end.pageX-_st.pageX,yL=_end.pageY-_st.pageY;
			if(!(xL&&yL)&&(inter<250||inter>=750)){//Tap
				$.fun.trigger.call($(target),inter<250?'tap':'longTap');
				touchTimeout = setTimeout(function(){
					cancelTouch();
				},250)
			}else{
				var _dir=swipeDirection(xL,yL);//Swipe
				if(_dir){
		...
		;['swipe', 'swipeLeft', 'swipeRight', 'tap', 'longTap'].forEach(function(eventName){
			$.fun[eventName] = function(callback){ return this.on(eventName, callback) }
		})
